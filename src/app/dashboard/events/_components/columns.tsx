...
export const eventColumns: ColumnDef<EventRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="text-sm font-medium">{format(d, 'do MMMM yyyy h:mm a')}</span>
      )
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="text-sm font-medium">{amount.pretty}</span>
      )
    },
  },
  {
          ...
          <div className="flex items-end justify-between gap-3">
            <div className="grid min-w-0 gap-0.5">
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm font-medium">{format(opens_at, 'do MMMM yyyy')}</span>

              </div>
              <div className="flex items-center gap-1">
                <span className="text-muted-foreground text-xs">Closes: </span>
                <span className="text-sm font-medium">{format(closes_at, 'do MMMM yyyy')}</span>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <span className="text-sm font-medium">{format(d, 'do MMMM yyyy h:mm a')}</span>
