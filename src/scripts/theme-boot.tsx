 ...
 *
 * Injected via `next/script` with `beforeInteractive` so preferences apply
 * before hydration without using a raw `<script>` in the React tree (React 19
 * warns that client-rendered `<script>` tags do not execute).
 */
...
export function ThemeBootScript() {
  ...
  // biome-ignore lint/security/noDangerouslySetInnerHtml: trusted inline boot IIFE for beforeInteractive injection
  return <Script id="theme-preferences-boot" strategy="beforeInteractive" dangerouslySetInnerHTML={{ __html: code }} />;
}
